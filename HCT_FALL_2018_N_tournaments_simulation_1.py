...
players = {'bloodyface':blo,'DacRyvius':Dac,'GoeLionKing':Goe,'Sintolol':Sin,'Caimiao':Cai,'Moyen':Moy,'Akumaker':Aku,'justsaiyan':jus,'LPTrunks':LPT,'Hatul':Hat,'lnguagehackr':Ing,'Bloodtrail':Blo,'Islandcat':Isl,'RENMEN':REN,'Tyler':Tyl,'Tincho':Tin}
player_list = ['bloodyface','DacRyvius','GoeLionKing','Sintolol','Caimiao','Moyen','Akumaker','justsaiyan','LPTrunks','Hatul','lnguagehackr','Bloodtrail','Islandcat','RENMEN','Tyler','Tincho']
...
def game(player1,player2):
    ...
    while (win1 < 3) & (win2 < 3):
        ...
        if deck1 == deck2:
            ...
            win_ratio = float(df[(df.player_archetype == deck1) & (df.opponent_archetype == deck2)]['win_rate']/100.0)
...
for i in range(0,1000):
    ...
    for pp in range(0,4):
        ...
        #print('First game',list(players.keys())[list(players.values()).index(p1)],list(players.keys())[list(players.values()).index(p2)])
        p1w,p1l = game(p1[:],p2[:])
        card_packs_given[list(players.keys())[list(players.values()).index(p1w)]] += 1
        #print(p1w,p1l)

        #print('Second game',list(players.keys())[list(players.values()).index(p3)],list(players.keys())[list(players.values()).index(p4)])
        p2w,p2l = game(p3[:],p4[:])
        card_packs_given[list(players.keys())[list(players.values()).index(p2w)]]+=1
        #print(p2w,p2l)

        #print('Third game',list(players.keys())[list(players.values()).index(p1w)],list(players.keys())[list(players.values()).index(p2w)])
        p2ww,p2wl = game(p1w[:],p2w[:])
        card_packs_given[list(players.keys())[list(players.values()).index(p2ww)]]+=1

        #print('Fourth game',list(players.keys())[list(players.values()).index(p1l)],list(players.keys())[list(players.values()).index(p2l)])
        p2lw,p2ll = game(p1l[:],p2l[:]) 
        card_packs_given[list(players.keys())[list(players.values()).index(p2lw)]]+=1

        #print('Fifth game',list(players.keys())[list(players.values()).index(p2wl)],list(players.keys())[list(players.values()).index(p2lw)])
    ...
    #print( champion_list )
    print('Calculate',i+1,'tournament simulation:',champion_list)
